...
const updateUser = [
    ...
    async (req, res, next) => {
        const allowedFields = ['currentPassword', 'password', 'local.pendingEmail', 'profile.displayName', 'profile.bio', 'profile.avatarUrl', 'username'];
    ...
    body('password').optional().trim().escape().isLength({ min: 8 }).withMessage('Password must be at least 8 characters long'),
    body('local.pendingEmail').optional().trim().escape().isEmail().normalizeEmail().withMessage('Invalid Email'),
    body('profile.displayName').optional().trim().escape().isLength({ max: 50 }).withMessage('Display name must be less than 50 characters long'),
    body('profile.bio').optional().trim().escape().isLength({ max: 160 }).withMessage('Bio must be less than 160 characters long'),
