...
class Game {
  ...
  get keywords() {
    ...
    return [this.gameCode, this.status, this.players.players.length, this.maxPlayers, ...displayNames];
  }
  ...
  placeCard(userId, cardIndexes, input) {
    ...
    if (
      this.jumpIn &&
      firstCard.colour === this.deck.topCard.colour &&
      firstCard.value === this.deck.topCard.value &&
