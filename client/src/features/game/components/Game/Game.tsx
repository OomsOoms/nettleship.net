...
const Game = ({
    ...
    const [pendingCard, setPendingCard] = useState<{ index: number } | null>(null)
    ...
    const calculatePlayerPositions = (count: number) => {
        ...
        if (isMobile) {
            ...
            if (totalPlayers <= 3) {
                ...
                for (let i = 0; i < totalPlayers; i++) {
                    const angle = startAngle + (endAngle - startAngle) * (i / (totalPlayers - 1))
                    positions.push({
        ...
        if (card.type === "wild" || card.type === "wildDrawFour") {
            ...
            if (gameSettings.stacking && card.type === "number") {
                ...
                const sameNumberCards = player.hand
                    .map((c, i) => (c.type === "number" && c.score === card.score && i !== index ? i : null))
    ...
    return (
        <div className="game">
            ...
            <div className="direction-indicator">
                ...
                {direction === 1 ? <RotateCw className="direction-icon" /> : <RotateCcw className="direction-icon" />}
            </div>
            ...
            <div className="card-piles">
                ...
                {/* Draw pile */}
                <div className="draw-pile" onClick={youPlayer.isTurn ? onDrawCard : undefined}>
                    <UnoCard card={{ colour: "black", type: "wild", score: 50 }} size={isMobile ? "md" : "lg"} faceDown={true} />
                </div>
            </div>
            ...
            {showPlayerSelector && (
                <div className="modal-overlay">
                    <PlayerSelector players={otherPlayers} handlePlayerSelect={handlePlayerSelect} />
                </div>
            )}
        </div>
    )
