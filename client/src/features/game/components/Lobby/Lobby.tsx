    ...
    <div className="lobby-wrapper">
      <div className="lobby__container">
        <div className="lobby__content">
          <div
            className={`lobby__layout ${isMobile ? "lobby__mobile-layout" : "lobby__desktop-layout"}`}
          >
            {/* Players section - adapts based on screen size */}
            <div
              className={`players-section ${!isMobile ? "players-section--desktop" : ""}`}
            >
              <div className="players-section__header">
                <h2
                  className={`players-section__title ${!isMobile ? "players-section__title--desktop" : ""}`}
                >
                  PLAYERS {players.length}/{gameSettings.maxPlayers}
                </h2>
                <div
                  className={`${!isMobile ? "players-section__select" : ""}`}
                >
                  <Select
              ...
              {isMobile ? (
                <div className="players-section__avatars">
                  ...
                  {Array.from(
                    { length: gameSettings.maxPlayers - players.length },
                    (_, index) =>
                      index === 0 && player.isHost ? (
                        <div
                          key={index + players.length}
                          className="players-section__add-bot"
                        >
                          <Button
                  ...
                  {Array.from(
                    ...
                    (_, index) =>
                      index === 0 && player.isHost ? (
                        <div
                          key={index + players.length}
                          className="players-section__add-bot-item"
                        >
                          <Button
                            onClick={handleAddBot}
                            className="players-section__add-bot-button"
            ...
            <div className="settings-section">
              ...
              <div className="settings-section__buttons">
                <Button
                  ...
                  onClick={() => {
                    ...
                    const inviteButton = document.querySelector(
                      ".settings-section__button--invite",
                    );
                    if (inviteButton) {
                ...
                {player.isHost && (
                  <Button
                    className="settings-section__button settings-section__button--start"
