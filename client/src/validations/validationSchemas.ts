...
export const validationSchemas: Record<string, Validator[]> = {
  username: [
    ...
    (value: string) =>
      value.length > 20 ? "username must be at most 20 characters" : null,
    (value: string) =>
      !/^[a-z0-9_.-]+$/.test(value)
  ...
  password: [
    ...
    (value: string) =>
      !/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d).{8,}$/.test(value)
        ? "password must include an uppercase, lowercase, and number"
        : null,
  ],
  newPassword: [
    (value: string) => (!value ? "password is required" : null),
  ...
  confirmPassword: [
    ...
    (value: string, compareValue?: string) =>
      value !== compareValue ? "passwords do not match" : null,
  ],
  gameCode: [
    (value: string) => (!value ? "game code" : null),
    (value: string) =>
      value.length !== 6 ? "game code must be 6 characters" : null,
    (value: string) =>
      !/^\d+$/.test(value) ? "game code must be numeric" : null,
  ],
};
